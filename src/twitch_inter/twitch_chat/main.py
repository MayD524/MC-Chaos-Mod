...
class mc_chaos_handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/":
            ...
            if len(tts_queue) == 0: self.set_headers(400); return
            self.set_headers(200)
            self.wfile.write(tts_queue[-1].encode())
            
            
        elif self.path == "/keystrokes":
            if len(otherEffects) == 0: self.set_headers(400); return
            self.set_headers(200)
            self.wfile.write(otherEffects[-1].encode())
...
if __name__ == '__main__':
    ...
    tts_queue:list[str]    = ["hello world!!!"]
    otherEffects:list[str] = ["a,b,c,d,mouse;100;100;1"]
